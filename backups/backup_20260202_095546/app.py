...
# Backup diário de arquivos críticos (uma vez por dia)
backup_automatico_diario(
    arquivos=[
        "config.yaml",
        "requirements.txt",
                        ...
                        with created_tabs[i]:
                            try:
                                ...
                                if tab_name == "👤 Meu Perfil":
                                    ...
                                    st.subheader("Métricas de Uso (Sessão Atual)")
                                    metrics = st.session_state.get("chat_metrics", {})
                                    duracoes = metrics.get("duracoes", [])
                                    total_perguntas = metrics.get("total", 0)
                                    ultima_resp = metrics.get("ultima_resposta_s")
                                    avg_resp = float(np.mean(duracoes)) if duracoes else 0.0
                                    p95_resp = float(np.percentile(duracoes, 95)) if len(duracoes) >= 5 else 0.0

                                    col_a, col_b, col_c, col_d = st.columns(4)
                                    col_a.metric("Perguntas (total)", total_perguntas)
                                    col_b.metric("Tempo médio (s)", f"{avg_resp:.2f}")
                                    col_c.metric("P95 (s)", f"{p95_resp:.2f}")
                                    col_d.metric("Última resposta (s)", f"{(ultima_resp or 0):.2f}")

                                    por_topico = metrics.get("por_topico", {})
                                    if por_topico:
                                        df_topicos = pd.DataFrame(
                                            [{"Tópico": k, "Perguntas": v} for k, v in sorted(por_topico.items(), key=lambda x: x[1], reverse=True)]
                                        )
                                    ...
                                    if perguntas:
                                        ...
                                        df_perguntas = pd.DataFrame(
                                            [{"Pergunta": k, "Qtd": v} for k, v in top_perguntas]
                                        )
                                        st.subheader("Perguntas mais comuns (Top 10)")
